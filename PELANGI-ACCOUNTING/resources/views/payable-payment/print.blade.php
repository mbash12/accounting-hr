...
<head>
    ...
    <style>
        ...
        .company-box-wrapper {
            margin-left: 80px; /* Offset to align with vendor box content */
            margin-bottom: 5px;
        }
        ...
        @media print {
            ...
            .container {
                width: 100%;
                /* height: 148mm;  Keep height constraint or let it flow? Keep rigid for "Half page" */
                height: 148mm; 
                margin: 0;
                padding: 5mm; 
                box-shadow: none;
...
<body>
    ...
    <div class="container {{ !$loop->last ? 'page-break' : '' }}">
        ...
        <div class="header-title-section">
            <div class="header-title">Purchase Payment</div>
        </div>
            ...
            <div class="left-column">
                ...
                <div class="vendor-wrapper">
                    ...
                    <div class="vendor-content">
                        <div class="info-box">
                            <div class="info-box-header">{{ $payment->supplier->name ?? '' }}</div>
                            <div class="info-box-content">
                                {!! nl2br(e($payment->supplier->address ?? '')) !!}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <table class="main-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $item->purchaseInvoice && $item->purchaseInvoice->date ? $item->purchaseInvoice->date->format('d/m/Y') : '-' }}</td>
                    <td>{{ $item->purchaseInvoice && $item->purchaseInvoice->due_date ? $item->purchaseInvoice->due_date->format('d/m/Y') : '-' }}</td>
                    <td style="text-align: right">{{ number_format($item->purchaseInvoice->total ?? 0, 0, ',', '.') }}</td>
                    <td style="text-align: right">{{ number_format(($item->purchaseInvoice->outstanding_amount ?? 0) + ($item->set_payment ?? 0), 0, ',', '.') }}</td>
        ...
        <div class="total-section">
            ...
            <div class="total-box">
                <div class="total-row">
                    <div>Total Owing:</div>
                    <div style="font-weight: bold;">{{ number_format($payment->items->sum(function($item) { return ($item->purchaseInvoice->outstanding_amount ?? 0) + ($item->set_payment ?? 0); }), 0, ',', '.') }}</div>
                </div>
                <div class="total-row">
