...
$drillAsOf = $drillEndDate ?? $date ?? null;
$canDrill = ! $account->is_header && filled($account->id) && $drillAsOf;
$drillUrl = $canDrill
    ? \App\Support\ReportDrilldown::generalLedgerUrlAsOf($account->id, $drillAsOf)
    : null;
@endphp
...
<tr style="{{ $account->is_header ? 'background-color: #f9fafb;' : '' }}">
    <td style="font-family: monospace; color: #4b5563;">
        <span style="background: #e5e7eb; padding: 0.25rem 0.5rem; border-radius: 0.25rem;">
            @if($drillUrl)
            <a href="{{ $drillUrl }}" class="report-drill-link" title="View General Ledger">{{ $account->code }}</a>
            @else
            {{ $account->code }}
            @endif
        </span>
