...
class PurchaseOrderController extends Controller
{
    public function storePurchaseOrder(Request $request)
    {
        ...
        $rules['purchase_order_no'] = 'nullable';
        $rules['date'] = 'required';
        $rules['reference_no'] = ''; 
        ...
        $rules['items.*.supplier_code'] = 'nullable';
        ...
        foreach ($post['items'] as $item) {
            $itemSupplierCode = $item['supplier_code'] ?? null;
            if (empty($itemSupplierCode)) {
                $itemSupplierCode = $request->supplier_code;
            }
            
            $supplier = null;
            $code = $itemSupplierCode ?: 'CT000012';
            if (!empty($code)) {
                $supplier = Contact::where('contact_code', $code)
                    ->where('company_id', $request->company_id)
                    ->first();
            }
            if (!$supplier) {
                $supplier = Contact::where('name', 'like', '%wisma%')
            ...
            $supplierCode = $supplier ? $supplier->contact_code : 'CT000012';
            
            $groupedItems[$supplierCode][] = [
                'item' => $item,
        ...
        try {
            ...
            $totalSubtotal = (float) $request->subtotal;
            reset($groupedItems);

            foreach ($groupedItems as $supplierCode => $groupData) {
                $groupSubtotal = 0;
                $groupTaxAmount = 0;
                foreach ($groupData as $gItem) {
                    $item = $gItem['item'];
                    $groupSubtotal += (float) $item['total'];
                    $groupTaxAmount += (float) ($item['tax_amount'] ?? 0);
                }

                $targetPoNo = $request->purchase_order_no;
                if ($targetPoNo) {
                    if (count($groupedItems) > 1) {
                        $targetPoNo = $targetPoNo . '-' . $supplierCode;
                    }
                }

                $po = $targetPoNo
                    ? \App\Models\PurchaseOrder::where('purchase_order_no', $targetPoNo)->where('company_id', $request->company_id)->first()
                    : null;
                ...
                if (!empty($po)) {
                    ...
                    if ($targetPoNo) {
                        $po->purchase_order_no = $targetPoNo;
                    }
                }

                $ratio = $totalSubtotal > 0 ? ($groupSubtotal / $totalSubtotal) : 1;
                $groupOtherCharges = ($request->other_charges ?? 0) * $ratio;
                $groupDiscount = ($request->discount ?? 0) * $ratio;

                $po->date = $request->date;
                $po->reference_no = $request->reference_no;
                $po->description = $request->description;
                $po->other_charges = $groupOtherCharges;
                $po->discount = $groupDiscount;
                $po->subtotal = $groupSubtotal;
                $po->tax_amount = $groupTaxAmount;
                $po->total = $groupSubtotal + $groupTaxAmount + $groupOtherCharges - $groupDiscount;
                $po->status = $request->status ?: 'draft';
                $po->supplier_id = $groupData[0]['supplier_id'];
        ...
        return response()->json([
            'code' => 200,
            'message' => 'submit po success',
            'data' => count($savedPOs) === 1 ? $savedPOs[0] : $savedPOs
        ]);
