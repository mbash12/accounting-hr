...
namespace App\Filament\Resources\AdvanceReceipts\Tables;
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make("toAccount.name")
                    ...
                    ->sortable(query: function (Builder $query, string $direction): Builder {
                        return $query->join('accounts', 'advance_receipts.to_account_id', '=', 'accounts.id')
                            ->orderBy('accounts.name', $direction)
                            ->select('advance_receipts.*');
                    }),
            ...
            ->recordActions([
                ActionGroup::make([
                    ...
                    EditAction::make()
                        ->disabled(fn($record) => in_array($record->status, ['processed', 'cancelled'], true))
                        ->tooltip(fn($record) => in_array($record->status, ['processed', 'cancelled'], true) ? __('Processed/cancelled transaction cannot be edited') : null),
                ]),
            ])
            ->toolbarActions([
