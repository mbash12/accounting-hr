...
import { getTokens } from "@/deps/firebase.js";
...
const grant = async () => {
    if ("Notification" in window) {
        ...
        if (permission === "granted") {
            ...
            setToken(currentUser.user?.id, state.token);
        } else {
            state.push = false;
        }
    } else {
...
const togglePush = async () => {
    if (state.push) {
        await deleteToken(currentUser.user?.id);
        state.push = false;
    } else {
        await grant();
    }
    localStorage.setItem("push", state.push);
};
onMounted(async () => {
    // return
    const pushed = localStorage.getItem("push");
    if (pushed) {
        if ("Notification" in window) {
...
const getData = async () => {
    ...
    state.notifications = [...state.notifications, ...notif.records];
    state.results = notif.results;
    loading(false);
};
...
const openNotif = async (notif) => {
    if (!notif.read_at) {
        notif.read_at = dayjs().format("YYYY-MM-DD HH:mm:ss");
        await readNotif([notif.id]);
    }
    router.push(`/${notif.notif_type}/${notif?.payload}`);
};
</script>
<template>
    <main>
        <div class="w-full h-full flex flex-col bg-white">
            ...
            <div
                class="flex-1 overflow-auto pb-60px"
                v-if="state.notifications?.length"
            >
                <div
                    class="border-b-4 p-4 flex flex-col gap-2"
                    v-for="(notif, i) in state.notifications"
                    :class="`${notif.readed_at ? 'opacity-50' : ''}`"
                    @click="openNotif(notif)"
                >
                    <div class="flex justify-between">
                        <span class="text-sm font-bold">{{ notif.title }}</span>
                ...
                <div
                    class="w-full h-50px border-t flex items-center justify-center"
                    v-if="state.results > state.notifications.length"
                >
                    <Button variant="white" class="text-14px" @click="loadMore"
            ...
            >
                <Button
                    ...
                    >Kembali</Button
                >
                <!-- <Button block variant="danger" class="text-xs"
                    >Tandai Sudah Baca</Button
                > -->
            </div>
        </div>
    </main>
